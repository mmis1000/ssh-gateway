...
FastReadStream.prototype._fetch = function _fetch() {
    ...
    this.channel._stream.readData(this.handle, state.result, 0, state.size, position, function onread(err, bytesRead, buffer, _position) {
        if (err) {
            return this.destroy(err);
        }
        ...
        if (state.got !== state.size) {
            this.channel._stream.readData(this.handle, state.result, state.got, state.size - state.got, position + state.got, onread.bind(this))
            // console.log(`#${state.index} next byte read ${state.got} ${state.size - state.got} ${position + state.got}`)
            return 
        }
